...
**Every project has a budget and records it in `AGENTS.md`.** A budget nobody wrote down is a budget
nobody is over. The figures below are a reasonable starting point, not the requirement — a
data-heavy internal tool and a marketing microsite do not share one number.

```yaml
# AGENTS.md
bundle-budget:
  landing: { js: 150kb, css: 30kb }    # gzipped
  app:     { js: 300kb, css: 50kb }
```

Revisit the budget when it is exceeded: either the addition is justified and the budget moves with a
recorded reason, or it is not and the addition comes out. Silently exceeding it is the one outcome
that is not allowed.
